...
# Which predictors
predictors <- c("duration_sleep", "efficiency_sleep", "subj_sleep", "social_jetlag")
...
sum_vars <- function(df){
  df %>% 
   mutate(psychotic_pq_sum = pq_1 + pq_2 + pq_3 + pq_4 + pq_5 + pq_6 + pq_7 + pq_8 +
         ...
         subj_sleep = sleep_1 + sleep_2 + sleep_3 + sleep_4 + 
...
tidy_df %>% 
  sum_vars() %>% 
  select(-c(id, psychotic_ysr_sum, pq_1:pq_16, ysr_1:ysr_3, sleep_1:sleep_8)) %>% 
...
non_response_df <-
  ...
  mutate(miss_pq = ifelse(sum_pq == 0, "Missing PQ", "Not missing"),
         ...
         miss_subj_sleep = ifelse(sum_sleep == 0, "Missing Sleep Self-report", "Not missing"),
  ...
  select(age, sex, ethnicity, mom_educt, mom_depr, psychotic_pq_sum, psychotic_ysr_dicht,
         duration_sleep:social_jetlag, subj_sleep, which_missing) 
...
non_response_df %>% 
  ...
  add_p() %>% 
  as_gt() %>% 
  tab_header("Table 2. Non-response patterns among predictor and outcome variables") %>% 
  opt_table_lines(extent = "none") %>%
  tab_options(
      heading.border.bottom.width = 2,
      heading.border.bottom.style = "solid",
...
2. Sum PQ items
3  Sum self-reported sleep problems
3. Square root transform self-reported sleep problems


```{r}
imputed_df <-
  mice_long %>% 
  mutate(subj_sleep = sleep_1 + sleep_2 + sleep_3 + sleep_4 + 
                      sleep_5 + sleep_6 + sleep_7 + sleep_8,
         subj_sleep_sqrt = sqrt(subj_sleep + 0.000001),
...
outliers_df <-
  ...
  mutate(id_out = 1:n(),
         ...
         # outlier_any = if_else((subj_sleep_outlier + duration_sleep_outlier +
...
coefs_df %>%   
  ...
  labs(x = "Psychotic experiences coefficient estimate", y = "",
       title = "Main predictor coefficients from every predictor and outcome model") +
...
adjusted_coeffs %>% 
  ...
  tab_footnote("Reference category for Mother's Education = Higher Education",
               location = cells_body(columns = vars(parameter),
...
# tbl_main_effects <-
  ...
  cols_label(coeff    = paste("Unstadardized", greekLetters::greeks("beta")),
             ...
             rsqr_ci  = "95% CI") %>% 
  tab_header(title = html("Table 3. Coefficient estimates and effect sizes of sleep predictors for each outcome and model fit (R<sup>2</sup>) for each full model")) %>% 
