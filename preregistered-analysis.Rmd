...
sim_sleep <-
  tibble(id = rep(as.character(1:n), each = 8),
         ...
         response  = sample(c(0, 1, 2, 3, 4), size = n*8, replace = T,
...
vars_miss <- c("ethnicity", "mom_depr", paste0("sleep_", 1:8),
...
non_response_df <-
  ...
  select(age, sex, ethnicity, mom_educt, mom_depr, psychotic_pq_sum, psychotic_ysr_dicht,
         duration_sleep:arousal_sleep, subj_sleep, which_missing) 
...
imputed_df <-
  ...
  mutate(subj_sleep = sleep_1 + sleep_2 + sleep_3 + sleep_4 + 
                      sleep_5 + sleep_6 + sleep_7 + sleep_8,
         subj_sleep_sqrt = sqrt(subj_sleep + 0.0001),
